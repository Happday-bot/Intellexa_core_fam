...
const DesignTeamDashboard = () => {
  ...
  // Ô£à Submit Poster Data (unchanged)
  ...
  const handleSubmitEvent = async (id) => {
    ...
    try {
      ...
      ...eventToUpdate,
      progressIndex: 3, // <ÔÇôÔÇô new field injected into the update lifecycle
    };
  ...
  return (
    <>
        ...
        <div className="bg-white shadow-2xl rounded-2xl p-6 border border-gray-100 mb-10">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-xl md:text-2xl font-semibold text-gray-700 flex items-center gap-2">
              ­ƒôè Monthly Poster Count Overview
            </h2>
          </div>
      ...
      {[
        { label: "Proposal", icon: "­ƒôä", link: event.proposal },
        { label: "Media", icon: "­ƒº¥", link: event.mediaFile },
        { label: "Form", icon: "­ƒôØ", link: event.formLink },
        { label: "Meet", icon: "­ƒÆ╗", link: event.meetLink },
      ].map((btn) => (
        <a
          key={btn.label}
          href={btn.link}
        ...
        >
          {btn.icon} {btn.label}
                  ...
                  <div className="flex flex-wrap gap-4 mt-6">
                    {[
                      { label: "Proposal", icon: "­ƒôä", link: event.proposal },
                      { label: "Marketting", icon: "­ƒº¥", link: event.marketingFile },
                      { label: "Form", icon: "­ƒôØ", link: event.formLink },
                      { label: "Meet", icon: "­ƒÆ╗", link: event.meetLink },
                    ].map((btn) => {
